...
with col2:
st.write("Contacta con [Dimoni](contactadimoni@gmail.com) para construir tus proyectos de IA personalizados.")
...
# Input
st.markdown("## Escribe el texto que quieres re-escribir")
...
if len(draft_input.split(" ")) > 700:
    st.write("Porfavor, introduce un texto con menos de 700 palabras.")
    st.stop()
...
with col1:
    option_tone = st.selectbox(
        '¿Qué tono te gustaría que tuviera tu texto?',
        ('Formal', 'Informal'))
    
with col2:
    option_dialect = st.selectbox(
        '¿Qué dialecto te gustaría que tuviera tu texto?',
        ('American', 'British'))
    
    
# Output
st.markdown("### Tu texto re-escrito:")

if draft_input:
    if not openai_api_key:
        st.warning('Porfavor introduce tu OpenAI API KEY. \
